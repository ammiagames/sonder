...
struct ProfileStatsServiceTests {
    ...
    @Test("NYC → LA → SF sums pairwise distances")
    func cumulativeDistance_consecutiveLogs() {
        let places = [
            TestData.place(id: "p1", name: "NYC", address: "123 St, New York, NY, USA",
                          latitude: 40.7128, longitude: -74.0060),
            TestData.place(id: "p2", name: "LA", address: "456 Ave, Los Angeles, CA, USA",
                          latitude: 34.0522, longitude: -118.2437),
            TestData.place(id: "p3", name: "SF", address: "789 Blvd, San Francisco, CA, USA",
                          latitude: 37.7749, longitude: -122.4194),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let logs = [
            TestData.log(id: "l1", placeID: "p1", createdAt: fixedDate()),
            TestData.log(id: "l2", placeID: "p2", createdAt: fixedDate().addingTimeInterval(3600)),
            TestData.log(id: "l3", placeID: "p3", createdAt: fixedDate().addingTimeInterval(7200)),
        ]

        let dist = ProfileStatsService.computeCumulativeDistance(logs: logs, placeMap: placeMap)

        let nycToLA = ProfileStatsService.haversineDistance(lat1: 40.7128, lng1: -74.0060, lat2: 34.0522, lng2: -118.2437)
        let laToSF = ProfileStatsService.haversineDistance(lat1: 34.0522, lng1: -118.2437, lat2: 37.7749, lng2: -122.4194)

        #expect(abs(dist.totalMiles - (nycToLA + laToSF)) < 1.0)
        #expect(dist.cityCount == 3)
    }

    @Test("Empty logs → 0 miles")
    func cumulativeDistance_emptyLogs() {
        let dist = ProfileStatsService.computeCumulativeDistance(logs: [], placeMap: [:])
        #expect(dist.totalMiles == 0)
        #expect(dist.cityCount == 0)
    }

    // MARK: - City Passport Stamps

    @Test("Multiple cities → sorted by count descending")
    func cityPassportStamps_multipleCities() {
        let citiesForPlaces: [String: String] = [
            "p1": "New York",
            "p2": "New York",
            "p3": "Los Angeles",
        ]
        let logs = [
            TestData.log(id: "l1", placeID: "p1"),
            TestData.log(id: "l2", placeID: "p2"),
            TestData.log(id: "l3", placeID: "p3"),
        ]

        let stamps = ProfileStatsService.computeCityPassportStamps(logs: logs, citiesForPlaces: citiesForPlaces)

        #expect(stamps.count == 2)
        #expect(stamps[0].cityName == "New York")
        #expect(stamps[0].logCount == 2)
        #expect(stamps[1].cityName == "Los Angeles")
        #expect(stamps[1].logCount == 1)
    }

    // MARK: - Neighborhood Coverage

    @Test("Different grid positions → unique zones counted")
    func neighborhoodCoverage_differentPositions() {
        // Places at different 0.01° grid cells in NYC
        let places = [
            TestData.place(id: "p1", address: "A, New York, NY, USA", latitude: 40.710, longitude: -74.000),
            TestData.place(id: "p2", address: "B, New York, NY, USA", latitude: 40.720, longitude: -74.010),
            TestData.place(id: "p3", address: "C, New York, NY, USA", latitude: 40.730, longitude: -74.020),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let citiesForPlaces = ["p1": "New York", "p2": "New York", "p3": "New York"]
        let logs = [
            TestData.log(id: "l1", placeID: "p1"),
            TestData.log(id: "l2", placeID: "p2"),
            TestData.log(id: "l3", placeID: "p3"),
        ...
        let coverage = ProfileStatsService.computeNeighborhoodCoverage(
            logs: logs, placeMap: placeMap, citiesForPlaces: citiesForPlaces)

        #expect(coverage != nil)
        #expect(coverage?.cityName == "New York")
        #expect(coverage?.uniqueZones == 3)
        #expect(coverage?.totalLogsInCity == 3)
    }

    @Test("Too few logs → nil")
    func neighborhoodCoverage_tooFewLogs() {
        let places = [TestData.place(id: "p1", address: "A, New York, NY, USA")]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let citiesForPlaces = ["p1": "New York"]
        let logs = [TestData.log(id: "l1", placeID: "p1")]
    ...
    @Test("Cumulative miles never decrease")
    func travelSparkline_monotonicallyIncreasing() {
        let places = [
            TestData.place(id: "p1", latitude: 40.7128, longitude: -74.0060),
            TestData.place(id: "p2", latitude: 34.0522, longitude: -118.2437),
            TestData.place(id: "p3", latitude: 37.7749, longitude: -122.4194),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let logs = [
            TestData.log(id: "l1", placeID: "p1", createdAt: fixedDate()),
        ...
        let sparkline = ProfileStatsService.computeTravelSparkline(logs: logs, placeMap: placeMap)

        #expect(sparkline.dataPoints.count == 3)
        for i in 1..<sparkline.dataPoints.count {
            #expect(sparkline.dataPoints[i].cumulativeMiles >= sparkline.dataPoints[i - 1].cumulativeMiles)
        }
    }

    // MARK: - Farthest From Home

    @Test("NYC (3 logs) + LA (1 log) → LA is farthest")
    func farthestFromHome_identifiesCorrectPlace() {
        let places = [
            TestData.place(id: "p1", name: "NYC Spot 1", address: "A, New York, NY, USA",
                          latitude: 40.7128, longitude: -74.0060),
            TestData.place(id: "p2", name: "NYC Spot 2", address: "B, New York, NY, USA",
                          latitude: 40.7580, longitude: -73.9855),
            TestData.place(id: "p3", name: "LA Spot", address: "C, Los Angeles, CA, USA",
                          latitude: 34.0522, longitude: -118.2437),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let citiesForPlaces = ["p1": "New York", "p2": "New York", "p3": "Los Angeles"]
        let logs = [
            TestData.log(id: "l1", placeID: "p1"),
            TestData.log(id: "l2", placeID: "p2"),
            TestData.log(id: "l3", placeID: "p1"),
            TestData.log(id: "l4", placeID: "p3"),
        ]

        let farthest = ProfileStatsService.computeFarthestFromHome(
            logs: logs, placeMap: placeMap, citiesForPlaces: citiesForPlaces)

        #expect(farthest != nil)
        #expect(farthest?.homeCityName == "New York")
        #expect(farthest?.placeName == "LA Spot")
        #expect(farthest?.distanceMiles ?? 0 > 2400)
    }

    @Test("Single city → nil")
    func farthestFromHome_singleCity() {
        let places = [
            TestData.place(id: "p1", address: "A, New York, NY, USA"),
            TestData.place(id: "p2", address: "B, New York, NY, USA"),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let citiesForPlaces = ["p1": "New York", "p2": "New York"]
        let logs = [
            TestData.log(id: "l1", placeID: "p1"),
            TestData.log(id: "l2", placeID: "p2"),
        ]
    ...
    func explorationScore_computation() {
        let places = [
            TestData.place(id: "p1", address: "A, New York, NY, USA", types: ["restaurant"]),
            TestData.place(id: "p2", address: "B, Los Angeles, CA, USA", types: ["cafe"]),
        ]
        let placeMap = Dictionary(uniqueKeysWithValues: places.map { ($0.id, $0) })
        let citiesForPlaces = ["p1": "New York", "p2": "Los Angeles"]
        let logs = [
            TestData.log(id: "l1", placeID: "p1"),
            TestData.log(id: "l2", placeID: "p2"),
            TestData.log(id: "l3", placeID: "p1"),
        ...
        let score = ProfileStatsService.computeExplorationScore(
            logs: logs, placeMap: placeMap, citiesForPlaces: citiesForPlaces)

        // 2 cities * 10 = 20, 1 country * 50 = 50, 3 logs * 1 = 3, 2 categories * 5 = 10
        #expect(score.cityPoints == 20)
        #expect(score.countryPoints == 50)
        #expect(score.logPoints == 3)
        #expect(score.categoryPoints == 10)
        #expect(score.score == 83)
        #expect(score.level == "Local")
    }

    @Test("Level thresholds are correct")
    func explorationScore_levelLabels() {
        // Test boundary values by constructing scores with known inputs
        // Newcomer: 0-49
        let newcomerScore = ExplorationScore(score: 10, level: "placeholder",
            cityPoints: 10, countryPoints: 0, logPoints: 0, categoryPoints: 0)
        // We test the service's level assignment via compute
        let places1 = [TestData.place(id: "p1", address: "A, CityA, SA, USA", types: ["restaurant"])]
        let placeMap1 = Dictionary(uniqueKeysWithValues: places1.map { ($0.id, $0) })
        let citiesForPlaces1 = ["p1": "CityA"]
        let logs1 = [TestData.log(id: "l1", placeID: "p1")]

        let score1 = ProfileStatsService.computeExplorationScore(
            logs: logs1, placeMap: placeMap1, citiesForPlaces: citiesForPlaces1)
        // 1 city * 10 + 1 country * 50 + 1 log + 1 cat * 5 = 66 → Local
        #expect(score1.level == "Local")

        // Empty → score 0 → Newcomer
        let score0 = ProfileStatsService.computeExplorationScore(
            logs: [], placeMap: [:], citiesForPlaces: [:])
        #expect(score0.score == 0)
        #expect(score0.level == "Newcomer")
        _ = newcomerScore // silence unused warning
    }
    ...
    @Test("Full compute returns valid ProfileStats with explorationStats")
    func fullCompute() {
        let places = [
        ...
        #expect(stats.explorationStats != nil)
        #expect(stats.explorationStats?.cumulativeDistance.totalMiles ?? 0 > 2400)
        #expect(stats.explorationStats?.explorationScore.score ?? 0 > 0)
        #expect(stats.explorationStats?.cityPassportStamps.count == 2)
    }
}
