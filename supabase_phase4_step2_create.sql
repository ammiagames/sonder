...
-- =====================================================
-- FOLLOWS TABLE (may already exist, that's ok)
-- =====================================================
...
CREATE POLICY "Users can view their followers" ON follows FOR SELECT USING (auth.uid() = following_id);
...
CREATE POLICY "Users can view own want_to_go" ON want_to_go FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can add to want_to_go" ON want_to_go FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own want_to_go" ON want_to_go FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete from want_to_go" ON want_to_go FOR DELETE USING (auth.uid() = user_id);


-- =====================================================
-- LOGS & USERS RLS UPDATES
-- =====================================================
...
CREATE POLICY "Users can view followed users logs" ON logs FOR SELECT
USING (
    auth.uid() = user_id
    OR EXISTS (SELECT 1 FROM follows WHERE follows.follower_id = auth.uid() AND follows.following_id = logs.user_id)
    OR EXISTS (SELECT 1 FROM users WHERE users.id = logs.user_id AND users.is_public = true)
);

CREATE POLICY "Users can view other users profiles" ON users FOR SELECT USING (true);
